...
export default function ChatBot() {
   ...
   const sendMessage = async () => {
     ...
     try {
       const response = await axios.post("https://codingassistant.onrender.com/prompt", { prompt: input });
       ...
       {isOpen && (
         <div className="bg-white w-80 h-95 shadow-lg rounded-xl p-4 border border-gray-300 flex flex-col">
           <div className="flex justify-between items-center">
             ...
             <h2 className="text-lg font-semibold"> <Bot size={24} />Nova</h2>
           ...
           <div className="mt-4 h-64 overflow-y-auto border p-2 rounded-lg bg-gray-100 flex flex-col">
             {messages.map((msg, index) => (
               <div key={index} className={`flex ${msg.sender === "user" ? "justify-end" : "justify-start"}`}>
                 <p className={`text-sm p-2 rounded-lg mb-2 max-w-[75%] ${msg.sender === "user" ? "bg-gray-300 text-purple-700" : "bg-gray-300 text-purple-700"}`}>
                   {msg.text}
                 </p>
               </div>
             ))}
             {isLoading && (
               <div className="flex justify-start">
                 <p className="text-sm p-2 rounded-lg mb-2 max-w-[75%] bg-gray-300 text-purple-600 animate-pulse">
                   Typing...
                 </p>
               </div>
             )}
           ...
           <div className="flex mt-2 border-t pt-2">
             ...
             <button 
               onClick={sendMessage} 
               className="ml-2 bg-purple-700 text-white p-2 rounded-lg hover:bg-purple-600 transition"
             >
               <Send size={20} />
             </button>
           </div>
